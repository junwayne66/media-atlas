...
**Current state: VF-003 (Persistence + Outbox) complete.** Layers so far: VF-001 skeleton (uv + pnpm workspaces, seven-service compose stack, CI) → VF-002 contracts (`packages/contracts-py` Pydantic truth → `schemas/` JSON Schema artifacts → `packages/contracts-ts` generated types; `make schemas` regenerates; TaskEnvelope rejects plaintext-credential keys; compat fixtures in `packages/contracts-py/tests/fixtures/v<N>/` are immutable) → VF-003 persistence (`packages/persistence`: SQLAlchemy 2 tables, repositories that only speak contract objects, optimistic locking via `version` column, transactional outbox + consumer-side `try_claim_event` dedupe; Alembic migrations at `migrations/`, `make migrate` targets the dev DB). Integration tests use testcontainers (own throwaway postgres; skip when docker unavailable; Ryuk disabled — this machine's Docker Desktop 500s on its socket-mount). Note: INSERT rowcount is unreliable under psycopg3 — use RETURNING to detect insertion. Git repo with trunk `main`; one task = one branch + `--no-ff` merge. Next roadmap task: VF-004 Artifact Store. `docs/` remains the implementation spec.
