...
def _fixture(vid: str, language: str, segs: list[tuple[int, int, str]],
              labels: list[LabelSpan]) -> HighlightFixture:
    return HighlightFixture(
        video_id=vid, language=language,
...
def _to_transcript(fx: HighlightFixture) -> Transcript:
    segs = [TranscriptSegment(id=f"{fx.video_id}-seg-{i}", start_ms=s, end_ms=e,
                                 language=fx.language, text=t, confidence=0.9)
            for i, (s, e, t) in enumerate(fx.segments)]
    return Transcript(
        id=f"tr-{fx.video_id}", language=fx.language, duration_ms=fx.duration_ms,
...
def _top3_for(fx: HighlightFixture) -> list[tuple[int, int]]:
    ...
    if not windows:
        return []
    specs = [HighlightWindowSpec(start_ms=w.start_ms, end_ms=w.end_ms, text=w.text,
    ...
    hl = rank_highlights(
        ...
        id_prefix=f"hl-{fx.video_id}", created_at=_T0,
        source_transcript_id=tr.id, feature_provider="fake",
    )
    return [(c.start_ms, c.end_ms) for c in hl.candidates]


def _hits(top3: list[tuple[int, int]], labels: list[LabelSpan],
           coverage_threshold: float = 0.7) -> int:
