...
INVALID_OVERRIDES: list[tuple[str, str, dict[str, Any]]] = [
            ...
            "slots": [
                {
                    ...
                    "role": "B_ROLL",
                    "query": "q",
                    "composition": {"aspect_ratio": "9:16", "safe_area": "center"},
    ...
    (
        "creative-timeline",
        "duration.rate 与 timeline.rate 必须一致",
    ...
    (
        "creative-timeline",
        "RationalTimeRange start/duration rate 不一致",
        {
            "tracks": [
                {
                    "id": "t",
                    "kind": "V1_PRIMARY_VIDEO",
                    "segments": [
                        {
                            "id": "s",
                            "time_range": {
    ...
    ("creative-timeline", "RationalTime value 不能为负", {"duration": {"value": -1, "rate": 30}}),
    ("render-manifest", "未知字段被拒", {"unexpected_field": 1}),
    (
        "render-manifest",
        "argv 元素含 shell 元字符被拒",
        {
            "render_graph": {
                **SAMPLES["render-manifest"].model_dump()["render_graph"],
    ...
    (
        "render-manifest",
        "input.sha256 非 64 hex 被拒",
        {
            "render_graph": {
                **SAMPLES["render-manifest"].model_dump()["render_graph"],
                "inputs": [{"asset_id": "a", "sha256": "ZZZ", "resolved_path": "/s/a.mp4"}],
            }
        },
    ),
    ...
    (
        ...
        "props 键非法（非 JS 标识符）被拒",
        {
            "request": {
                **SAMPLES["remotion-render-manifest"].model_dump()["request"],
    ...
    (
        "remotion-render-manifest",
        "entry_component_path 含 shell 元字符被拒",
        {
            "request": {
                **SAMPLES["remotion-render-manifest"].model_dump()["request"],
            ...
            "sentences": [
                {
                    ...
                    "source_text": "x",
                    "semantic_intent": "i",
                    "source_time_range_start_ms": 100,
                    "source_time_range_end_ms": 0,
                    "target_duration_ms": 100,
                }
            ]
        },
    ...
    (
        "glossary",
        "preserve_source 与 target_term 不一致被拒",
        {"entries": [{"source_term": "Qwen", "target_term": "QwenX", "preserve_source": True}]},
    ),
    # 术语表：entries source_term 重复被拒
    (
        "glossary",
        "entries source_term 重复被拒",
        {
            "entries": [
    ...
    (
        "translate-reflect-adapt-result",
        "semantic_similarity 越界被拒",
    ...
    (
        ...
        "sentence semantic_similarity 负值被拒",
        {
            "sentences": [
                {
                    "id": "ls-x",
    ...
    (
        ...
        "无阅读速度上限被拒",
        {
            "cjk_chars_per_sec": None,
            "en_chars_per_sec": None,
            ...
            "cues": [
                {
                    ...
                    "start_ms": 5000,
                    "end_ms": 1000,
                    "lines": [{"text": "x", "start_ms": 0, "end_ms": 100, "language": "zh-CN"}],
                }
            ],
        },
    ),
    ...
    (
        ...
        "AUTHORIZED_INPAINT 无 license_ref 被拒",
        {
            "clean_plate_requests": [
                {
                    "id": "cp-x",
                    "source_track_id": "tt-0",
    ...
    (
        "text-localization-plan",
        "decision 引用未知 clean_plate id 被拒",
        {
            "decisions": [
                {
                    "source_track_id": "tt-x",
                    "source_text": "x",
                    "source_kind": "CAPTION",
                    "target_language": "en-US",
                    "strategy": "REDRAW",
    ...
    (
        "text-localization-plan",
        "重复的 source_track_id 被拒",
        {
            "decisions": [
                {
                    "source_track_id": "same",
                    "source_text": "a",
                    "source_kind": "CAPTION",
                    "target_language": "en-US",
                    "strategy": "SKIP",
    ...
    (
        "text-localization-plan",
        "policy expansion ratio ≤ 1 不属于本合同（by design "
    ...
    (
        "voice-profile",
        "CLONED 无 sample_source_ref 被拒",
        {
            "voice_kind": "CLONED",
            "sample_source_ref": None,
    ...
    ("voice-profile", "非法 license_status 被拒", {"license_status": "APPROVED"}),
    # TTS：display_name 空
    ...
    (
        ...
        "ducked_by 悬空被拒",
        {
            "tracks": [
                ...
                {"id": "m", "kind": "MUSIC", "start_ms": 0, "end_ms": 100, "ducked_by": "ghost"},
            ],
        },
    ),
    # 音频合成：轨自己 Ducking 自己
            ...
            "decisions": [
                {
                    ...
                    "end_ms": 100,
                    "eligible": False,
                    "ineligible_reasons": ["MULTIPLE_FACES"],
                    "method": "GPU_SYNTHESIS",
                    "rationale": "x",
                }
            ],
        },
    ...
    (
        "template-trust-state",
        "recent_fatal 超窗口被拒",
        {
            "stats": {
                "approved_render_count": 5,
                "recent_fatal_count": 99,
    ...
    ("platform-publish-spec", "min_width > max_width 被拒", {"min_width": 2000, "max_width": 1080}),
    # 预检：min_duration > max_duration
    (
        "platform-publish-spec",
        "min_duration > max_duration 被拒",
        {"min_duration_ms": 700000, "max_duration_ms": 600000},
    ),
    # 预检：allowed_aspect_ratios 不能空
    ("platform-publish-spec", "allowed_aspect_ratios 空被拒", {"allowed_aspect_ratios": []}),
    # 预检：publishable=True 不能与 ERROR/FATAL 并存
    (
        "preflight-report",
        "publishable 却含 ERROR 被拒",
        {
            "publishable": True,
            "findings": [
                {
    ...
    ("publish-connector-capability", "max_file_size_bytes ≤ 0 被拒", {"max_file_size_bytes": 0}),
    # 发布任务：SUCCEEDED 必须携带 external_post_id
    (
        "publish-job",
        "SUCCEEDED 无 external_post_id 被拒",
        {"state": "SUCCEEDED", "external_post_id": None},
    ),
    # 发布任务：SUCCEEDED_RECONCILED 同理
    (
        "publish-job",
        "SUCCEEDED_RECONCILED 无 external_post_id 被拒",
    ...
    ("platform-account", "内嵌 refresh_token 被拒", {"credential_ref": "handle_refresh_token_abc"}),
    ...
    (
        "snapshot-schedule",
        "captured 非 planned 子集被拒",
        {"planned_ages_hours": [1.0, 3.0], "captured_ages_hours": [6.0]},
    ),
    ("metrics-connector-capability", "非法 MetricField 被拒", {"provided_fields": ["FOO"]}),
    ("metrics-connector-capability", "min_seconds 为负被拒", {"min_seconds_between_calls": -1}),
    ("video-performance-record", "account_id 空被拒", {"account_id": ""}),
    ("video-performance-record", "publish_hour 超 23 被拒", {"features": {"publish_hour": 24}}),
    (
        "account-baseline",
        "sample_count 为负被拒",
    ...
    (
        ...
        "有相对值却无基线中位数被拒",
        {
            "baseline": {"age_hours": 24.0, "metric": "VIEWS", "median": None, "sample_count": 0},
            "group_stats": [
                {
                    "dimension": "TEMPLATE",
                    "value": "t",
    ...
    ("learning-signal-result", "correlation 超 1 被拒", {"correlation": 1.5}),
    ("learning-signal-result", "association_only False 被拒", {"association_only": False}),
    (
        "learning-signal-result",
        "样本不足却给方向被拒",
        {"enough_samples": False, "direction": "POSITIVE", "sample_count": 2},
    ),
    ("learning-signal-result", "非法 SignalKind 被拒", {"signal": "MOON_PHASE"}),
    ("learning-report", "min_samples < 1 被拒", {"min_samples": 0}),
    ("learning-report", "account_id 空被拒", {"account_id": ""}),
    ("ranker-weights", "系数为负被拒", {"coefficients": {"hook_strength": -0.1}}),
    ("ranker-weights", "系数为空被拒", {"coefficients": {}}),
    (
        "calibration-proposal",
        "EXPLORE 却 promotable=False 被拒",
        {"promotable": False, "decision": "EXPLORE", "exploration_fraction": 0.1},
    ),
    ("calibration-proposal", "探索比例超硬上限被拒", {"exploration_fraction": 0.5}),
    (
        "calibration-proposal",
        "放宽 max 探索上限被拒（只能收紧）",
    ...
    (
        "calibration-proposal",
        "非 EXPLORE 却带探索流量被拒",
        {"decision": "KEEP_CURRENT", "exploration_fraction": 0.1, "promotable": False},
    ),
    (
        "calibration-proposal",
        "非法 CalibrationDecision 被拒",
        {"decision": "FULL_ROLLOUT", "exploration_fraction": 0.0, "promotable": False},
    ),
    ("source-asset", "未知字段被拒", {"unexpected_field": 1}),
    ("source-asset", "file_sha256 必须是 64 位十六进制", {"file_sha256": "ZZZ"}),
    ("source-asset", "version 从 1 起", {"version": 0}),
    ("source-asset", "处置理由不能为空", {"reason": ""}),
    (
        "source-asset",
        "IMPORTED 却无 file_sha256 被拒",
        {"disposition": "IMPORTED", "file_sha256": None},
    ),
    (
        "source-asset",
        "UNRESOLVABLE 却无 error_code 被拒",
        {"disposition": "UNRESOLVABLE", "error_code": None},
    ),
    ("source-asset", "非法处置枚举被拒", {"disposition": "DOWNLOADED"}),
    (
        "source-asset",
        "attempts 缺 connector 被拒",
        {"acquisition": {"tool_name": "download.router", "attempts": [{"status": "unconfigured"}]}},
    ),
    ...
    (
        "exporter-report",
        "非法 ExporterStatus 被拒",
        {"entries": [{"kind": "OTIO_FILE", "status": "SUCCESS"}]},
    ),
    (
        "exporter-report",
        "output_path 含 shell 元字符被拒",
        {"entries": [{"kind": "OTIO_FILE", "status": "OK", "output_path": "/output/$(id).otio"}]},
    ),
    (
        "exporter-report",
        "bytes_written 不能为负",
