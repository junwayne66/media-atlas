...
def _zh_tpl(**overrides) -> SubtitleTemplate:
    defaults = dict(
        id="tpl", language="zh-CN",
        max_chars_per_line=16, max_lines_per_cue=2,
...
def _en_tpl(**overrides) -> SubtitleTemplate:
    defaults = dict(
        id="tpl-en", language="en-US",
        max_chars_per_line=42, max_lines_per_cue=2,
...
def test_pack_single_line() -> None:
    cue = pack_lines_into_cue(
        "短句", start_ms=0, end_ms=1000, language="zh-CN",
        cue_id="c-0", template=_zh_tpl(),
    )
...
def test_pack_multiple_lines_share_duration() -> None:
    cue = pack_lines_into_cue(
        "这是一段较长的中文字幕内容",
        start_ms=1000, end_ms=5000, language="zh-CN",
        cue_id="c-0", template=_zh_tpl(max_chars_per_line=6, max_lines_per_cue=2),
    )
...
def test_pack_over_max_lines_merges_tail() -> None:
    ...
    cue = pack_lines_into_cue(
        ...
        start_ms=0, end_ms=3000, language="en-US",
        cue_id="c-0", template=_en_tpl(max_chars_per_line=10, max_lines_per_cue=2),
    )
    assert len(cue.lines) <= 2
...
def _clean_cue(cue_id: str, start: int, end: int, text: str = "短句") -> SubtitleCue:
    return SubtitleCue(
        id=cue_id, start_ms=start, end_ms=end,
...
def _clean_track(cues: list[SubtitleCue]) -> SubtitleTrack:
    return SubtitleTrack(
        id="tr", language="zh-CN", template_id="tpl", cues=cues, created_at=_T0,
    )
...
def test_validate_reading_speed_en_row_with_cjk_char_still_uses_en_rules() -> None:
    ...
    tr = SubtitleTrack(
        id="tr", language="en-US", template_id="tpl-en",
        cues=[SubtitleCue(id="c0", start_ms=0, end_ms=500, lines=[
            # 含一个 CJK 字符的 en 行；en_chars_per_sec 应仍然约束
            SubtitleLine(text="the very quick brown fox 世 jumps over lazy dog",
                          start_ms=0, end_ms=500, language="en-US"),
        ])],
        created_at=_T0,
    )
...
def test_validate_reading_speed_en() -> None:
    tr = SubtitleTrack(
        id="tr", language="en-US", template_id="tpl-en",
        cues=[SubtitleCue(id="c0", start_ms=0, end_ms=500, lines=[
            SubtitleLine(text="the very quick brown fox jumps over lazy dog rapidly",
                          start_ms=0, end_ms=500, language="en-US"),
        ])],
        created_at=_T0,
    )
...
def test_validate_line_too_long() -> None:
    tr = _clean_track([SubtitleCue(
        id="c0", start_ms=0, end_ms=2000,
        lines=[SubtitleLine(text="一" * 20, start_ms=0, end_ms=2000, language="zh-CN")],
    )])
    kinds = [i.kind for i in validate_subtitle_track(
        tr, template=_zh_tpl(max_chars_per_line=16),
    )]
...
def test_validate_too_many_lines() -> None:
    ...
    kinds = [i.kind for i in validate_subtitle_track(
        tr, template=_zh_tpl(max_lines_per_cue=2),
    )]
...
def test_validate_word_out_of_line_span() -> None:
    tr = _clean_track([SubtitleCue(
        id="c0", start_ms=0, end_ms=2000,
        lines=[SubtitleLine(
            text="hello", start_ms=100, end_ms=1000, language="zh-CN",
...
def test_render_srt_format() -> None:
    tr = _clean_track([_clean_cue("c0", 0, 2000, text="第一句"),
                          _clean_cue("c1", 3000, 5000, text="第二句")])
    out = render_srt(tr)
...
def test_safe_area_bounds_ok() -> None:
    spec = SafeAreaSpec(left_min_pct=5, right_max_pct=95,
                          top_min_pct=5, bottom_max_pct=90)
