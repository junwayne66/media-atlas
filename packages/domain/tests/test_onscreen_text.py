...
def _bbox(**k):
    ...
    return BBox(x=k.get("x", 0.1), y=k.get("y", 0.85), w=k.get("w", 0.6), h=k.get("h", 0.08))


def _track(
    tid: str = "tt-0",
    kind: TextTrackKind = TextTrackKind.CAPTION,
    text: str = "端侧推理 20ms 完成",
...
def test_brand_mark_always_skip_even_if_policy_says_redraw() -> None:
    ...
    d = decide_strategy(
        _track(kind=TextTrackKind.BRAND_MARK, text="Qwen"),
...
def test_unknown_kind_goes_to_skip_review() -> None:
    d = decide_strategy(_track(kind=TextTrackKind.UNKNOWN, text="???"), policy=_policy())
    assert d.strategy is TextLocalizationStrategy.SKIP
    assert d.needs_review
...
def test_ui_requires_license_check_falls_to_annotation() -> None:
    d = decide_strategy(
        _track(kind=TextTrackKind.UI, text="Save"), policy=_policy(), translated_text="保存"
    )
...
def test_caption_high_confidence_low_occlusion_goes_redraw() -> None:
    d = decide_strategy(_track(), policy=_policy(), translated_text="On-device 20 ms")
...
def test_caption_low_confidence_falls_back_to_info_card() -> None:
    d = decide_strategy(
        _track(confidence=0.5, low_confidence=True), policy=_policy(), translated_text="x"
    )
...
def test_caption_high_occlusion_falls_back() -> None:
    ...
    d = decide_strategy(
        _track(occluded_frames=2), policy=_policy(max_occlusion=0.3), translated_text="x"
    )
    assert d.strategy is TextLocalizationStrategy.INFO_CARD_FALLBACK
...
def test_caption_layout_overflow_falls_back() -> None:
    d = decide_strategy(
        _track(), policy=_policy(max_expansion=1.3), translated_text="x", layout_expansion_ratio=1.5
    )
    assert d.strategy is TextLocalizationStrategy.INFO_CARD_FALLBACK


def test_scene_text_default_keep() -> None:
    d = decide_strategy(_track(kind=TextTrackKind.SCENE_TEXT, text="Coffee"), policy=_policy())
...
def test_validate_translated_text_missing_for_redraw() -> None:
    p = _clean_plan(
        [_decision(translated_text=None, clean_plate_request_id="cp-0")],
        requests=[
            CleanPlateRequest(
                id="cp-0",
                source_track_id="tt-0",
...
def test_validate_translated_text_illegal_for_keep_as_is() -> None:
    p = _clean_plan(
        [_decision(strategy=TextLocalizationStrategy.KEEP_AS_IS, translated_text="不该给")]
    )
    kinds = [i.kind for i in validate_text_localization_plan(p, policy=_policy())]
...
def test_validate_layout_overflow() -> None:
    p = _clean_plan(
        [_decision(layout_expansion_ratio=1.5, clean_plate_request_id="cp-0")],
        requests=[
            CleanPlateRequest(
                id="cp-0",
                source_track_id="tt-0",
    ...
    kinds = [i.kind for i in validate_text_localization_plan(p, policy=_policy(max_expansion=1.3))]
...
def test_validate_glossary_preserve_source_lost() -> None:
    ...
    kinds = [i.kind for i in validate_text_localization_plan(p, policy=_policy(), glossary=gl)]
...
def test_is_valid_true_on_clean_plan() -> None:
    tts = TextTrackSet(
        id="tts",
        tracks=[_track(tid="c0", kind=TextTrackKind.CAPTION)],
        ocr_provider="fake",
        created_at=_T0,
    )
    p = plan_text_localization(
