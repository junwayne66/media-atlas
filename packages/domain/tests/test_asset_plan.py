...
def _slot(
    sid: str = "slot-0",
    *,
    allowed=(AssetSource.SOURCE, AssetSource.OWN_LIBRARY),
    start=0,
    end=5000,
...
def test_resolver_reuse_penalty_across_slots() -> None:
    ...
    slots = [
        _slot("s0", allowed=RESOLVER_PRIORITY),
...
def test_resolver_deterministic_on_ties() -> None:
    ...
    a = resolve_asset_plan(
        [_slot(allowed=(AssetSource.OWN_LIBRARY,))], cands, plan_id="p", created_at=_T0
    )
    b = resolve_asset_plan(
        [_slot(allowed=(AssetSource.OWN_LIBRARY,))], cands, plan_id="p", created_at=_T0
    )
...
def _resolved(
    ...
    *,
    ...
    return ResolvedAsset(
        ...
        license=lic,
        query="q",
        usage_start_ms=usage_start,
        usage_end_ms=usage_end,
        match_score=0.7,
    )
...
def test_source_not_allowed_flagged() -> None:
    ...
    plan = _plan(
        [_slot(allowed=(AssetSource.SOURCE,))],
...
def test_usage_out_of_slot_flagged() -> None:
    plan = _plan(
        [_slot(start=0, end=5000)], [_resolved("slot-0", usage_start=0, usage_end=8000)]
    )  # 超出 slot
    kinds = {i.kind for i in validate_asset_plan(plan)}
...
def test_license_expired_flagged() -> None:
    expired = AssetLicense(
        type=AssetLicenseType.LICENSED_STOCK, holder="stock.co", valid_until=_T0 - timedelta(days=1)
    )
    plan = _plan(
        [_slot(allowed=(AssetSource.STOCK,))],
...
def test_reuse_exceeded_flagged() -> None:
    ...
    resolved = [
        ResolvedAsset(
            ...
            license=_OWNED,
            query="q",
            usage_start_ms=i * 1000,
            usage_end_ms=(i + 1) * 1000,
            match_score=0.7,
        )
        for i in range(4)
    ]
