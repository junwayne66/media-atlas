...
def _rec(rid, views, *, qa=None, edits=None, selected=None, music=None,
          hotness=None, acct="a1") -> VideoPerformanceRecord:
    f = PerformanceFeatures(qa_warning_count=qa, manual_edit_count=edits,
                             human_selected=selected, has_music=music,
                             trend_hotness_at_publish=hotness)
    snaps = ([PerformanceSnapshot(id="s", platform=_TK, platform_post_id="p",
                                    account_id=acct, observed_at=_T, age_hours=24.0,
                                    views=views, source_confidence=1.0)]
             if views is not None else [])
    return VideoPerformanceRecord(id=rid, account_id=acct, platform=_TK,
                                   published_at=_T, features=f, snapshots=snaps)
...
def test_negative_association_and_association_only():
    ...
    res = analyze_signal(recs, signal=SignalKind.QA_WARNING_COUNT,
                          baseline_median=550.0, age_hours=24.0,
                          metric=MetricField.VIEWS, min_samples=8)
...
def test_small_sample_is_insufficient_no_direction():
    ...
    res = analyze_signal(recs, signal=SignalKind.QA_WARNING_COUNT,
                          baseline_median=150.0, age_hours=24.0,
                          metric=MetricField.VIEWS, min_samples=8)
...
def test_null_pairs_excluded_never_zero():
    ...
    res = analyze_signal(recs, signal=SignalKind.QA_WARNING_COUNT,
                          baseline_median=250.0, age_hours=24.0,
                          metric=MetricField.VIEWS, min_samples=8)
...
def test_boolean_signal_buckets_yes_no():
    ...
    res = analyze_signal(recs, signal=SignalKind.HUMAN_SELECTED,
                          baseline_median=500.0, age_hours=24.0,
                          metric=MetricField.VIEWS, min_samples=3)
...
def test_numeric_signal_buckets_terciles():
    ...
    res = analyze_signal(recs, signal=SignalKind.QA_WARNING_COUNT,
                          baseline_median=140.0, age_hours=24.0,
                          metric=MetricField.VIEWS, min_samples=3)
...
def test_build_report_is_account_scoped():
    ...
    rep = build_learning_report(a + b, account_id="a1", platform=_TK,
                                 age_hours=24.0, metric=MetricField.VIEWS,
                                 generated_at=_T, min_samples=8)
...
def test_significant_signals_only_directional_and_association():
    ...
    rep = build_learning_report(recs, account_id="a1", platform=_TK,
                                 age_hours=24.0, metric=MetricField.VIEWS,
                                 generated_at=_T, min_samples=8)
    sig = significant_signals(rep)
    assert all(s.direction in (SignalDirection.POSITIVE, SignalDirection.NEGATIVE)
               for s in sig)
...
def _result(**over) -> LearningSignalResult:
    base = dict(signal=SignalKind.QA_WARNING_COUNT, metric=MetricField.VIEWS,
                age_hours=24.0, correlation=0.5, direction=SignalDirection.POSITIVE,
                sample_count=10, enough_samples=True)
    base.update(over)
    return LearningSignalResult(**base)


def _report(results) -> LearningReport:
    return LearningReport(account_id="a1", platform=_TK, generated_at=_T,
                           age_hours=24.0, metric=MetricField.VIEWS,
                           min_samples=8, signals=results)
