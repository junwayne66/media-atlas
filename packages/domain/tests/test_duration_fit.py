...
def test_rewrite_preferred_when_it_lands_in_band():
    ...
    d = decide_duration_fit(_d("s", estimated_ms=3600, target_ms=3000, rewritten_estimate_ms=3100))
    assert d.status is DurationFitStatus.OK_FITTED
...
def test_rewrite_still_out_of_band_falls_through():
    ...
    d = decide_duration_fit(
        _d("s", estimated_ms=3600, target_ms=3000, rewritten_estimate_ms=3500, broll_slack_ms=0)
    )
...
def test_broll_absorbs_overflow():
    ...
    d = decide_duration_fit(_d("s", estimated_ms=3600, target_ms=3000, broll_slack_ms=500))
    assert d.status is DurationFitStatus.OK_FITTED
...
def test_broll_fills_deficit_when_too_short():
    ...
    d = decide_duration_fit(_d("s", estimated_ms=2000, target_ms=3000, broll_slack_ms=1000))
    assert d.status is DurationFitStatus.OK_FITTED
    assert d.fit_method is DurationFitStrategy.BROLL_ADJUST
...
def test_face_locked_forbids_time_stretch():
    ...
    d = decide_duration_fit(_d("s", estimated_ms=3300, target_ms=3000, face_locked=True))
...
def test_validate_flags_status_method_inconsistency():
    ...
    inconsistent = [i for i in issues if i.kind is DurationFitIssueKind.STATUS_METHOD_INCONSISTENT]
