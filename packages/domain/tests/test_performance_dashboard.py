...
def _rec(
    rid, views, *, tmpl="T1", dur=45000, acct="a1", hour=20, mode=CreationMode.STRUCTURE_REWRITE
) -> VideoPerformanceRecord:
    feats = PerformanceFeatures(
        template_id=tmpl, creation_mode=mode, duration_ms=dur, language="zh-CN", publish_hour=hour
    )
    snaps = [_snap(views)] if views is not None else []
    return VideoPerformanceRecord(
        id=rid, account_id=acct, platform=_TK, published_at=_T, features=feats, snapshots=snaps
    )
...
def test_small_sample_never_ranked():
    ...
    rare = next(
        g for g in d.group_stats if g.dimension is GroupDimension.TEMPLATE and g.value == "RARE"
    )
...
def _dash_with(groups: list[PerformanceGroupStat], *, median=9000.0, min_samples=5):
    return PerformanceDashboard(
        ...
        baseline=AccountBaselineEntry(
            age_hours=24.0, metric=MetricField.VIEWS, median=median, sample_count=20
        ),
...
def test_cross_account_records_guard():
    recs = [_rec("a", 100), _rec("x", 200, acct="other"), _rec("y", 300, acct="a1")]
