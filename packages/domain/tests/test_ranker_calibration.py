...
def _w(version="v1", **coeffs) -> RankerWeights:
    return RankerWeights(ranker_kind=_HL, template_version=version,
                          coefficients=coeffs or {"a": 1.0, "b": 0.1})
...
def _samples(n=24, *, drive="b"):
    ...
    out = []
    for i in range(n):
        feats = ({"a": float(n - i), "b": float(i)} if drive == "b"
                 else {"a": float(i), "b": float(n - i)})
        out.append(RankingSample(item_id=f"i{i}", features=feats,
...
def test_evaluate_ranking_null_outcome_excluded():
    s = _samples(24) + [RankingSample(item_id="nx", features={"a": 1.0, "b": 1.0},
                                       relative_outcome=None)]
...
def test_propose_calibration_bounded_and_directional():
    ...
    samples = [RankingSample(item_id=f"i{i}", features={"good": float(i), "bad": float(24 - i)},
                              relative_outcome=float(i)) for i in range(24)]
...
def test_propose_calibration_stays_non_negative():
    cur = _w("v1", x=0.05)
    samples = [RankingSample(item_id=f"i{i}", features={"x": float(24 - i)},
                              relative_outcome=float(i)) for i in range(24)]
...
def test_decide_explore_when_candidate_better():
    ...
    prop = decide_calibration(cur, cand, _samples(24, drive="b"), generated_at=_T,
                               min_samples=20, min_improvement=0.05,
...
def test_decide_insufficient_samples():
    prop = decide_calibration(_w(), _w("v2"), _samples(5), generated_at=_T,
                               min_samples=20)
...
def test_exploration_fraction_capped():
    ...
    prop = decide_calibration(cur, cand, _samples(24, drive="b"), generated_at=_T,
                               min_samples=20, min_improvement=0.0,
                               exploration_fraction=0.9, max_exploration_fraction=0.2)
...
def test_no_full_rollout_decision_exists():
    ...
    assert {d.value for d in CalibrationDecision} == {
        "KEEP_CURRENT", "EXPLORE", "INSUFFICIENT_SAMPLES"}


def test_contract_rejects_inflated_max_exploration():
    # verifier REFUTED 回归：max_exploration_fraction 只能收紧、绝不能放宽到硬上限之上
    ev = RankingEvalResult(ranker_kind=_HL, template_version="v1", top_k=3,
                            rank_correlation=0.5, sample_count=24, enough_samples=True)
    import pytest
    for bad in ({"max_exploration_fraction": 1.0, "exploration_fraction": 1.0},
                {"max_exploration_fraction": 0.5, "exploration_fraction": 0.3},
                {"max_exploration_fraction": 0.2, "exploration_fraction": 0.3}):
        with pytest.raises(ValueError):
            CalibrationProposal(
                ranker_kind=_HL, current=_w(), candidate=_w("v2"),
                current_eval=ev, candidate_eval=ev, improvement=2.0, promotable=True,
                decision=CalibrationDecision.EXPLORE, generated_at=_T, **bad)
...
def test_engine_clamps_exploration_against_hard_cap():
    ...
    prop = decide_calibration(cur, cand, _samples(24, drive="b"), generated_at=_T,
                               min_samples=20, min_improvement=0.0,
                               exploration_fraction=0.9, max_exploration_fraction=1.0)
...
def test_validate_catches_promotable_without_samples():
    ...
    thin_eval = RankingEvalResult(ranker_kind=_HL, template_version="v1", top_k=3,
                                   rank_correlation=0.5, sample_count=3,
                                   enough_samples=False)
    prop = CalibrationProposal(
        ranker_kind=_HL, current=_w(), candidate=_w("v2"),
        current_eval=thin_eval, candidate_eval=thin_eval, improvement=0.1,
        promotable=True, decision=CalibrationDecision.KEEP_CURRENT,
        exploration_fraction=0.0, generated_at=_T,
    )
