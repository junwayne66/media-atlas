...
def _table(claims: list[tuple[str, str, ClaimSourceStatus]]) -> ClaimTable:
    return ClaimTable(
        id="ct", source_blueprint_id="bp",
        entries=[
            ClaimTableEntry(claim_id=cid, text=t, fact_status=st,
...
def _canonical(sents: list[CanonicalSentence]) -> CanonicalScript:
    return CanonicalScript(
        id="cs", source_language="zh-CN", sentences=sents,
        total_duration_ms=sum(s.target_duration_ms for s in sents),
        created_at=_T0,
    )


def _make_cs(sid: str = "cs-0", claims: list[str] | None = None,
              must_keep: list[str] | None = None, text: str = "端侧模型 20ms 完成推理",
              dur: int = 3000) -> CanonicalSentence:
    return CanonicalSentence(
        id=sid, beat_slot_id="b", role=RhetoricalBeatKind.EVIDENCE,
        source_language="zh-CN", source_text=text, semantic_intent="evidence",
        claim_ids=claims or [],
        source_time_range_start_ms=0, source_time_range_end_ms=dur,
        target_duration_ms=dur, must_keep_terms=must_keep or [],
    )


def _make_ls(sid: str, canonical_id: str, text: str, dur: int = 3000,
              sim: float = 0.9, claims: list[str] | None = None,
              lang: str = "en-US") -> LocalizedSentence:
    return LocalizedSentence(
        id=sid, canonical_sentence_id=canonical_id, target_language=lang,
        text=text, duration_estimate_ms=dur, semantic_similarity=sim,
        claim_ids=claims or [],
    )
...
def test_claim_diff_added_and_removed() -> None:
    ct = _table([("c1", "x", ClaimSourceStatus.VERIFIED),
                  ("c2", "y", ClaimSourceStatus.VERIFIED)])
...
def test_validate_empty_variant_fails() -> None:
    ...
    var = LocalizationVariant(
        id="v", canonical_script_id="cs", target_language="en-US",
        sentences=[], provider="tra.fake", created_at=_T0,
    )
    ct = _table([])
...
def test_validate_language_mismatch() -> None:
    ...
    var = LocalizationVariant(
        id="v", canonical_script_id="cs", target_language="en-US",
        sentences=[_make_ls("ls-0", "cs-0", "on-device 20 ms",
                             lang="ja-JP")],
        provider="fake", created_at=_T0,
    )
    ct = _table([])
...
def test_validate_must_keep_case_insensitive() -> None:
    ...
    kinds_ok = [i.kind for i in validate_localization(var_ok, canon,
                                                          claim_table=_table([]))]
    assert LocalizationIssueKind.MUST_KEEP_TERM_LOST not in kinds_ok
    # 反例：'brain' 不能命中 'AI'
    var_bad = LocalizationVariant(
        id="v2", canonical_script_id="cs", target_language="en-US",
        sentences=[_make_ls("ls-0", "cs-0", "the brain chip")],
        provider="fake", created_at=_T0,
    )
    kinds_bad = [i.kind for i in validate_localization(var_bad, canon,
                                                           claim_table=_table([]))]
...
def test_validate_glossary_preserve_source_violation() -> None:
    ...
    kinds = [i.kind for i in validate_localization(var, canon,
                                                       claim_table=_table([]), glossary=gl)]
...
def test_is_valid_localization_true_on_clean() -> None:
    ...
    var = LocalizationVariant(
        id="v", canonical_script_id="cs", target_language="en-US",
        sentences=[_make_ls("ls-0", "cs-0", "the Qwen on-device model runs fast",
                             claims=["c1"], dur=3000, sim=0.92)],
        provider="fake", created_at=_T0,
    )
    ct = _table([("c1", "x", ClaimSourceStatus.VERIFIED)])
...
def test_build_canonical_from_script_uses_transcript_ids_when_present() -> None:
    ...
    tr = Transcript(
        id="tr", language="zh-CN", duration_ms=10000,
        segments=[
            TranscriptSegment(id="s0", start_ms=0, end_ms=4000, language="zh-CN",
                                text="端侧模型", confidence=0.9),
            TranscriptSegment(id="s1", start_ms=4000, end_ms=10000, language="zh-CN",
                                text="推理 20ms", confidence=0.9),
        ],
        models=TranscriptModels(asr_provider="fake"), created_at=_T0,
    )
    script = ScriptVersion(
        id="sv", language="zh-CN", version=1,
        sentences=[
            ScriptSentence(id="s0", beat_slot_id="b", role=RhetoricalBeatKind.HOOK,
                             text="端侧模型登场", target_duration_ms=4000,
                             claim_ids=["c1"], language="zh-CN"),
            ScriptSentence(id="s1", beat_slot_id="b", role=RhetoricalBeatKind.EVIDENCE,
                             text="20ms 完成推理", target_duration_ms=6000,
                             claim_ids=["c1"], language="zh-CN"),
        ],
        created_at=_T0,
    )
    canon = build_canonical_script(
        script, claim_table=_table([("c1", "x", ClaimSourceStatus.VERIFIED)]),
        canonical_id="c-1", created_at=_T0, source_transcript=tr,
...
def test_build_canonical_falls_back_to_cursor_when_no_transcript() -> None:
    ...
    script = ScriptVersion(
        id="sv", language="zh-CN", version=1,
        sentences=[
            ScriptSentence(id="a", beat_slot_id="b", role=RhetoricalBeatKind.HOOK,
                             text="x", target_duration_ms=2000, language="zh-CN"),
            ScriptSentence(id="b", beat_slot_id="b", role=RhetoricalBeatKind.CTA,
                             text="y", target_duration_ms=3000, language="zh-CN"),
        ],
        created_at=_T0,
    )
    canon = build_canonical_script(script, claim_table=_table([]),
                                       canonical_id="c-1", created_at=_T0)
    assert canon.sentences[0].source_time_range_start_ms == 0
...
def test_validate_sentence_mismatch_missing_translation() -> None:
    ...
    # 具体指向 cs-1
    mismatches = [i for i in issues
                    if i.kind is LocalizationIssueKind.SENTENCE_MISMATCH]
...
def test_must_keep_ai_between_cjk_matches() -> None:
    ...
    var = LocalizationVariant(
        id="v", canonical_script_id="cs", target_language="zh-CN",
