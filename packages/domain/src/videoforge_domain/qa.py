...
def detect_black_frames(
    frames: list[VideoFrameSample], thresholds: QAThresholds = DEFAULT_THRESHOLDS,
) -> list[QAFinding]:
        ...
        if frame.mean_luma <= thresholds.black_luma_max:
            ...
            if run_start is not None:
                findings.append(QAFinding(
                    id=f"blk-{idx}", kind=QAFindingKind.BLACK_FRAME,
                    severity=QASeverity.BLOCKER, at_ms=run_start,
    ...
    for i in range(1, len(frames)):
        if frames[i].phash == frames[i - 1].phash:
            ...
            if run_len >= thresholds.frozen_min_frames:
                findings.append(QAFinding(
                    id=f"frz-{idx}", kind=QAFindingKind.FROZEN_FRAME,
                    severity=QASeverity.MAJOR, at_ms=run_start,
                    duration_ms=frames[i - 1].at_ms - run_start,
                    evidence={"frame_count": run_len,
...
def detect_duplicate_frames(
    frames: list[VideoFrameSample], _: QAThresholds = DEFAULT_THRESHOLDS,
) -> list[QAFinding]:
    ...
    for i in range(1, len(frames)):
        if frames[i].phash == frames[i - 1].phash:
            findings.append(QAFinding(
                id=f"dup-{i}", kind=QAFindingKind.DUPLICATE_FRAMES,
                severity=QASeverity.MINOR, at_ms=frames[i].at_ms,
                duration_ms=0,
...
def check_loudness(
    audio_windows: list[AudioWindowSample], thresholds: QAThresholds = DEFAULT_THRESHOLDS,
) -> list[QAFinding]:
    ...
    if longest.lufs < lo or longest.lufs > hi:
        findings.append(QAFinding(
            id="loud-0", kind=QAFindingKind.LOUDNESS_OUT_OF_RANGE,
            severity=QASeverity.MAJOR, at_ms=longest.at_ms,
    ...
    for i, w in enumerate(audio_windows):
        if w.true_peak_dbtp > thresholds.true_peak_max_dbtp:
            findings.append(QAFinding(
                id=f"tp-{i}", kind=QAFindingKind.TRUE_PEAK_CLIP,
                severity=QASeverity.BLOCKER, at_ms=w.at_ms, duration_ms=w.duration_ms,
                evidence={"true_peak_dbtp": w.true_peak_dbtp,
...
def detect_voice_tail_cut(
    audio_windows: list[AudioWindowSample], _: QAThresholds = DEFAULT_THRESHOLDS,
) -> list[QAFinding]:
    ...
    if last.has_voice and last.tail_amplitude > 0.05:
        return [QAFinding(
            id="tail-0", kind=QAFindingKind.VOICE_TAIL_CUT,
            severity=QASeverity.BLOCKER, at_ms=last.at_ms + last.duration_ms,
            duration_ms=0,
...
def check_caption_safe_area(
    captions: list[CaptionBoundingBox], thresholds: QAThresholds = DEFAULT_THRESHOLDS,
) -> list[QAFinding]:
    """任意边越出安全区（<min 或 >max %）→ CAPTION_OFF_SAFE_AREA MINOR。id 加索引防重复。"""
    findings: list[QAFinding] = []
    for i, cap in enumerate(captions):
        if (cap.left_pct < thresholds.safe_area_min_pct
            ...
            findings.append(QAFinding(
                id=f"safe-{i}-{cap.caption_id}", kind=QAFindingKind.CAPTION_OFF_SAFE_AREA,
                severity=QASeverity.MINOR, at_ms=cap.at_ms, duration_ms=cap.duration_ms,
                track_ref="v4",
                evidence={"left": cap.left_pct, "right": cap.right_pct,
                            "top": cap.top_pct, "bottom": cap.bottom_pct,
    ...
    for i in range(len(captions)):
        for j in range(i + 1, len(captions)):
            ...
            if a.at_ms < b_end and b.at_ms < a_end:
                findings.append(QAFinding(
                    id=f"cap-ov-{i}-{j}",
                    kind=QAFindingKind.CAPTION_OVERLAP, severity=QASeverity.MAJOR,
...
def check_broll_ratio(
    broll_duration_ms: int, total_video_duration_ms: int,
    ...
    if ratio < thresholds.broll_min_ratio:
        return [QAFinding(
            id="broll-0", kind=QAFindingKind.BROLL_RATIO_LOW,
            severity=QASeverity.MINOR, at_ms=0, duration_ms=total_video_duration_ms,
...
def check_duration_match(
    timeline_duration_ms: int, measured_duration_ms: int,
    thresholds: QAThresholds = DEFAULT_THRESHOLDS,
) -> list[QAFinding]:
    ...
    if diff > thresholds.duration_tolerance_ms:
        return [QAFinding(
            id="dur-0", kind=QAFindingKind.DURATION_MISMATCH,
            severity=QASeverity.BLOCKER, at_ms=0, duration_ms=diff,
            evidence={"timeline_ms": timeline_duration_ms,
...
def aggregate_qa_report(
    ...
    *,
    ...
    findings.extend(check_broll_ratio(
        input_data.broll_duration_ms, input_data.total_video_duration_ms, thresholds,
    ))
    findings.extend(check_duration_match(
        timeline_duration_ms, input_data.measured_duration_ms, thresholds,
    ))

    gate = validate_publish_gate(findings, thresholds=thresholds)
    return QAReport(
        id=report_id, timeline_id=timeline_id, render_manifest_id=render_manifest_id,
        findings=findings,
...
def validate_publish_gate(
    findings: list[QAFinding], *, thresholds: QAThresholds = DEFAULT_THRESHOLDS,
) -> PublishGate:
