...
def _guard_output_path(path: str, allowed_roots: tuple[str, ...]) -> Path:
    ...
    if any(c in _SHELL_META_CHARS for c in path):
        ...
        raise UnsafeInputPath(f"output_path 含不允许的 shell 元字符 {offenders!r}：{path}")
    ...
    *,
    ...
    try:
        ...
        return ExportEntry(
            ...
            tool_version=_OTIO_WRITER_VERSION,
        ...
        return ExportEntry(
            kind=ExporterKind.OTIO_FILE,
            status=ExporterStatus.FAILED,
            tool_version=_OTIO_WRITER_VERSION,
            errors=[f"OTIO 写入失败：{exc!r}"],
        )
...
def _timeline_to_fcpxml_root(timeline: CreativeTimeline) -> ET.Element:
    ...
    ET.SubElement(
        ...
        {
            "id": "r1",
            "name": f"FFVideoFormat{timeline.rate}",
            "frameDuration": f"1/{timeline.rate}s",
            "width": "1080",
            "height": "1920",
        },
    )
    ...
    *,
    ...
    try:
        ...
        return ExportEntry(
            ...
            status=ExporterStatus.OK,
            output_path=str(target),
            tool_version=_FCPXML_WRITER_VERSION,
...
def _write_experimental_package(
    ...
    try:
        ...
        payload = json.dumps(
            json.loads(timeline.model_dump_json()), ensure_ascii=False, indent=2, sort_keys=True
        )
        ...
        return ExportEntry(
            ...
            status=ExporterStatus.FAILED,
            tool_version=tool_version,
            errors=[f"{target_editor} 包写入失败：{exc!r}"],
        )
...
def write_jianying_package(
    ...
    *,
    ...
    return _write_experimental_package(
        ...
        output_path,
        allowed_roots,
        ExporterKind.JIANYING,
        _JIANYING_WRITER_VERSION,
        "剪映 JianYing",
    )
