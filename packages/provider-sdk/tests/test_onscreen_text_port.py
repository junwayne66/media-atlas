...
def _cp_request(
    method: CleanPlateMethod = CleanPlateMethod.BACKGROUND_ESTIMATE, license_ref: str | None = None
) -> CleanPlateRequest:
...
def test_fake_redraw_ok_when_ratio_within_bound() -> None:
    r = FakeTextRedrawProvider().redraw(
        TextRedrawRequest(
            decision=_decision(source_text="A" * 10, translated="X" * 12),
            clean_plate_artifact_id="cp-fake-1",
