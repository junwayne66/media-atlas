...
def _job(window: str = "immediate"):
    return new_publish_job(
        id="j", account_id="a", platform=PublishPlatform.DOUYIN,
...
def test_auth_errors_map_to_auth_required_human():
    for code, msg in [("2190008", "invalid access_token"),
                       ("", "授权过期，请重新授权"), ("", "unauthorized"),
                       ("", "token expired")]:
...
def test_mapping_is_case_insensitive():
    assert map_publish_error("", "ACCESS_TOKEN EXPIRED").status is (
...
def test_fake_douyin_error_injections():
    assert FakeDouyinPublishExecutor(challenge=True).submit(
        _job()).status is PublishExecStatus.CHALLENGE
    assert FakeDouyinPublishExecutor(auth_required=True).submit(
        _job()).status is PublishExecStatus.AUTH_REQUIRED
    assert FakeDouyinPublishExecutor(rate_limited=True).submit(
        _job()).error_code is PublishExecErrorCode.RATE_LIMITED
...
def test_error_mapping_module_zero_network():
    ...
    allowed = {"__future__", "dataclasses", "enum", "typing",
                "videoforge_provider_sdk", "videoforge_contracts"}
