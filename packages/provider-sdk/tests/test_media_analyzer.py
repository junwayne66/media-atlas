...
def test_fake_end_to_end_clean_input_passes_gate() -> None:
    ...
    fake = FakeMediaAnalyzerProvider(
        ...
        audio_windows=[AudioWindow(at_ms=0, duration_ms=45000, lufs=-14.0,
                                     true_peak_dbtp=-3.0, has_voice=False, tail_amplitude=0.001)],
        captions=[CaptionBBox(caption_id="c0", at_ms=0, duration_ms=1500,
                                 left_pct=10, top_pct=80, right_pct=90, bottom_pct=92)],
        measured_duration_ms=45000, broll_duration_ms=10000, total_video_duration_ms=45000,
    )
    ...
    input_data = MediaSampleInput(
        video_frames=[VideoFrameSample(at_ms=s.at_ms, mean_luma=s.mean_luma, phash=s.phash)
                        for s in result.frame_samples],
        audio_windows=[AudioWindowSample(at_ms=w.at_ms, duration_ms=w.duration_ms,
                                            lufs=w.lufs, true_peak_dbtp=w.true_peak_dbtp,
                                            has_voice=w.has_voice, tail_amplitude=w.tail_amplitude)
                        for w in result.audio_windows],
        captions=[CaptionBoundingBox(
            caption_id=c.caption_id, at_ms=c.at_ms, duration_ms=c.duration_ms,
            left_pct=c.left_pct, top_pct=c.top_pct, right_pct=c.right_pct, bottom_pct=c.bottom_pct,
...
def test_fake_end_to_end_black_frame_blocks_gate() -> None:
    # 黑帧样本 → BLOCKER → BLOCK
    fake = FakeMediaAnalyzerProvider(
        frame_samples=[FrameSample(at_ms=0, mean_luma=100, phash="a"),
